"use strict";
var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toESM = (mod, isNodeMode, target) => (target = mod != null ? __create(__getProtoOf(mod)) : {}, __copyProps(
  // If the importer is in node compatibility mode or this is not an ESM
  // file that has been converted to a CommonJS file using a Babel-
  // compatible transform (i.e. "__esModule" has not been set), then set
  // "default" to the CommonJS "module.exports" for node compatibility.
  isNodeMode || !mod || !mod.__esModule ? __defProp(target, "default", { value: mod, enumerable: true }) : target,
  mod
));
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/components/index.ts
var index_exports = {};
__export(index_exports, {
  TimelineMap: () => TimelineMap,
  convertTime: () => convertTime,
  default: () => TimelineMap,
  findClosestTimezone: () => findClosestTimezone
});
module.exports = __toCommonJS(index_exports);

// src/components/TimelineMap.tsx
var import_react_leaflet = require("react-leaflet");
var import_leaflet = __toESM(require("leaflet"));
var import_react = __toESM(require("react"));
var import_luxon2 = require("luxon");

// src/lib/timezone.ts
var import_fast_levenshtein = __toESM(require("fast-levenshtein"));
var import_luxon = require("luxon");

// src/lib/timezoneLocations.ts
var timezoneLocations = [
  {
    "timezone": "Europe/Andorra",
    "latitude": 42.5,
    "longitude": 1.516667
  },
  {
    "timezone": "Asia/Dubai",
    "latitude": 25.3,
    "longitude": 55.3
  },
  {
    "timezone": "Asia/Kabul",
    "latitude": 34.516667,
    "longitude": 69.2
  },
  {
    "timezone": "Europe/Tirane",
    "latitude": 41.333333,
    "longitude": 19.833333
  },
  {
    "timezone": "Asia/Yerevan",
    "latitude": 40.183333,
    "longitude": 44.5
  },
  {
    "timezone": "Antarctica/Casey",
    "latitude": -66.283333,
    "longitude": 110.516667,
    "comments": "Casey"
  },
  {
    "timezone": "Antarctica/Davis",
    "latitude": -68.583333,
    "longitude": 77.966667,
    "comments": "Davis"
  },
  {
    "timezone": "Antarctica/DumontDUrville",
    "latitude": -66.666667,
    "longitude": 140.016667,
    "comments": "Dumont-d'Urville"
  },
  {
    "timezone": "Antarctica/Mawson",
    "latitude": -67.6,
    "longitude": 62.883333,
    "comments": "Mawson"
  },
  {
    "timezone": "Antarctica/Palmer",
    "latitude": -64.8,
    "longitude": -64.1,
    "comments": "Palmer"
  },
  {
    "timezone": "Antarctica/Rothera",
    "latitude": -67.566667,
    "longitude": -68.133333,
    "comments": "Rothera"
  },
  {
    "timezone": "Antarctica/Syowa",
    "latitude": -69.006111,
    "longitude": 39.59,
    "comments": "Syowa"
  },
  {
    "timezone": "Antarctica/Troll",
    "latitude": -72.011389,
    "longitude": 2.535,
    "comments": "Troll"
  },
  {
    "timezone": "Antarctica/Vostok",
    "latitude": -78.4,
    "longitude": 106.9,
    "comments": "Vostok"
  },
  {
    "timezone": "America/Argentina/Buenos_Aires",
    "latitude": -34.6,
    "longitude": -58.45,
    "comments": "Buenos Aires (BA, CF)"
  },
  {
    "timezone": "America/Argentina/Cordoba",
    "latitude": -31.4,
    "longitude": -64.183333,
    "comments": "Argentina (most areas: CB, CC, CN, ER, FM, MN, SE, SF)"
  },
  {
    "timezone": "America/Argentina/Salta",
    "latitude": -24.783333,
    "longitude": -65.416667,
    "comments": "Salta (SA, LP, NQ, RN)"
  },
  {
    "timezone": "America/Argentina/Jujuy",
    "latitude": -24.183333,
    "longitude": -65.3,
    "comments": "Jujuy (JY)"
  },
  {
    "timezone": "America/Argentina/Tucuman",
    "latitude": -26.816667,
    "longitude": -65.783333,
    "comments": "Tucum\xE1n (TM)"
  },
  {
    "timezone": "America/Argentina/Catamarca",
    "latitude": -28.466667,
    "longitude": -65.783333,
    "comments": "Catamarca (CT); Chubut (CH)"
  },
  {
    "timezone": "America/Argentina/La_Rioja",
    "latitude": -29.433333,
    "longitude": -66.85,
    "comments": "La Rioja (LR)"
  },
  {
    "timezone": "America/Argentina/San_Juan",
    "latitude": -31.533333,
    "longitude": -68.516667,
    "comments": "San Juan (SJ)"
  },
  {
    "timezone": "America/Argentina/Mendoza",
    "latitude": -32.883333,
    "longitude": -68.816667,
    "comments": "Mendoza (MZ)"
  },
  {
    "timezone": "America/Argentina/San_Luis",
    "latitude": -33.316667,
    "longitude": -66.35,
    "comments": "San Luis (SL)"
  },
  {
    "timezone": "America/Argentina/Rio_Gallegos",
    "latitude": -51.633333,
    "longitude": -69.216667,
    "comments": "Santa Cruz (SC)"
  },
  {
    "timezone": "America/Argentina/Ushuaia",
    "latitude": -54.8,
    "longitude": -68.3,
    "comments": "Tierra del Fuego (TF)"
  },
  {
    "timezone": "Pacific/Pago_Pago",
    "latitude": -14.266667,
    "longitude": -170.7,
    "comments": "Samoa, Midway"
  },
  {
    "timezone": "Europe/Vienna",
    "latitude": 48.216667,
    "longitude": 16.333333
  },
  {
    "timezone": "Australia/Lord_Howe",
    "latitude": -31.55,
    "longitude": 159.083333,
    "comments": "Lord Howe Island"
  },
  {
    "timezone": "Antarctica/Macquarie",
    "latitude": -54.5,
    "longitude": 158.95,
    "comments": "Macquarie Island"
  },
  {
    "timezone": "Australia/Hobart",
    "latitude": -42.883333,
    "longitude": 147.316667,
    "comments": "Tasmania"
  },
  {
    "timezone": "Australia/Melbourne",
    "latitude": -37.816667,
    "longitude": 144.966667,
    "comments": "Victoria"
  },
  {
    "timezone": "Australia/Sydney",
    "latitude": -33.866667,
    "longitude": 151.216667,
    "comments": "New South Wales (most areas)"
  },
  {
    "timezone": "Australia/Broken_Hill",
    "latitude": -31.95,
    "longitude": 141.45,
    "comments": "New South Wales (Yancowinna)"
  },
  {
    "timezone": "Australia/Brisbane",
    "latitude": -27.466667,
    "longitude": 153.033333,
    "comments": "Queensland (most areas)"
  },
  {
    "timezone": "Australia/Lindeman",
    "latitude": -20.266667,
    "longitude": 149,
    "comments": "Queensland (Whitsunday Islands)"
  },
  {
    "timezone": "Australia/Adelaide",
    "latitude": -34.916667,
    "longitude": 138.583333,
    "comments": "South Australia"
  },
  {
    "timezone": "Australia/Darwin",
    "latitude": -12.466667,
    "longitude": 130.833333,
    "comments": "Northern Territory"
  },
  {
    "timezone": "Australia/Perth",
    "latitude": -31.95,
    "longitude": 115.85,
    "comments": "Western Australia (most areas)"
  },
  {
    "timezone": "Australia/Eucla",
    "latitude": -31.716667,
    "longitude": 128.866667,
    "comments": "Western Australia (Eucla)"
  },
  {
    "timezone": "Asia/Baku",
    "latitude": 40.383333,
    "longitude": 49.85
  },
  {
    "timezone": "Atlantic/Bermuda",
    "latitude": 32.283333,
    "longitude": -64.766667
  },
  {
    "timezone": "Asia/Dhaka",
    "latitude": 23.716667,
    "longitude": 90.416667
  },
  {
    "timezone": "Europe/Brussels",
    "latitude": 50.833333,
    "longitude": 4.333333
  },
  {
    "timezone": "Europe/Sofia",
    "latitude": 42.683333,
    "longitude": 23.316667
  },
  {
    "timezone": "Atlantic/Cape_Verde",
    "latitude": 14.916667,
    "longitude": -23.516667
  },
  {
    "timezone": "America/Belize",
    "latitude": 17.5,
    "longitude": -88.2
  },
  {
    "timezone": "America/St_Johns",
    "latitude": 47.566667,
    "longitude": -52.716667,
    "comments": "Newfoundland; Labrador (southeast)"
  },
  {
    "timezone": "America/Halifax",
    "latitude": 44.65,
    "longitude": -63.6,
    "comments": "Atlantic - NS (most areas); PE"
  },
  {
    "timezone": "America/Glace_Bay",
    "latitude": 46.2,
    "longitude": -59.95,
    "comments": "Atlantic - NS (Cape Breton)"
  },
  {
    "timezone": "America/Moncton",
    "latitude": 46.1,
    "longitude": -64.783333,
    "comments": "Atlantic - New Brunswick"
  },
  {
    "timezone": "America/Goose_Bay",
    "latitude": 53.333333,
    "longitude": -60.416667,
    "comments": "Atlantic - Labrador (most areas)"
  },
  {
    "timezone": "America/Blanc-Sablon",
    "latitude": 51.416667,
    "longitude": -57.116667,
    "comments": "AST - QC (Lower North Shore)"
  },
  {
    "timezone": "America/Toronto",
    "latitude": 43.65,
    "longitude": -79.383333,
    "comments": "Eastern - ON, QC (most areas)"
  },
  {
    "timezone": "America/Nipigon",
    "latitude": 49.016667,
    "longitude": -88.266667,
    "comments": "Eastern - ON, QC (no DST 1967-73)"
  },
  {
    "timezone": "America/Thunder_Bay",
    "latitude": 48.383333,
    "longitude": -89.25,
    "comments": "Eastern - ON (Thunder Bay)"
  },
  {
    "timezone": "America/Iqaluit",
    "latitude": 63.733333,
    "longitude": -68.466667,
    "comments": "Eastern - NU (most east areas)"
  },
  {
    "timezone": "America/Pangnirtung",
    "latitude": 66.133333,
    "longitude": -65.733333,
    "comments": "Eastern - NU (Pangnirtung)"
  },
  {
    "timezone": "America/Atikokan",
    "latitude": 48.758611,
    "longitude": -91.621667,
    "comments": "EST - ON (Atikokan); NU (Coral H)"
  },
  {
    "timezone": "America/Winnipeg",
    "latitude": 49.883333,
    "longitude": -97.15,
    "comments": "Central - ON (west); Manitoba"
  },
  {
    "timezone": "America/Rainy_River",
    "latitude": 48.716667,
    "longitude": -94.566667,
    "comments": "Central - ON (Rainy R, Ft Frances)"
  },
  {
    "timezone": "America/Resolute",
    "latitude": 74.695833,
    "longitude": -94.829167,
    "comments": "Central - NU (Resolute)"
  },
  {
    "timezone": "America/Rankin_Inlet",
    "latitude": 62.45,
    "longitude": -92.716667,
    "comments": "Central - NU (central)"
  },
  {
    "timezone": "America/Regina",
    "latitude": 50.4,
    "longitude": -104.65,
    "comments": "CST - SK (most areas)"
  },
  {
    "timezone": "America/Swift_Current",
    "latitude": 50.283333,
    "longitude": -107.833333,
    "comments": "CST - SK (midwest)"
  },
  {
    "timezone": "America/Edmonton",
    "latitude": 53.55,
    "longitude": -113.466667,
    "comments": "Mountain - AB; BC (E); SK (W)"
  },
  {
    "timezone": "America/Cambridge_Bay",
    "latitude": 69.116667,
    "longitude": -105.033333,
    "comments": "Mountain - NU (west)"
  },
  {
    "timezone": "America/Yellowknife",
    "latitude": 62.45,
    "longitude": -114.35,
    "comments": "Mountain - NT (central)"
  },
  {
    "timezone": "America/Inuvik",
    "latitude": 68.316667,
    "longitude": -133.783333,
    "comments": "Mountain - NT (west)"
  },
  {
    "timezone": "America/Creston",
    "latitude": 49.1,
    "longitude": -116.516667,
    "comments": "MST - BC (Creston)"
  },
  {
    "timezone": "America/Dawson_Creek",
    "latitude": 59.766667,
    "longitude": -120.233333,
    "comments": "MST - BC (Dawson Cr, Ft St John)"
  },
  {
    "timezone": "America/Fort_Nelson",
    "latitude": 58.8,
    "longitude": -122.7,
    "comments": "MST - BC (Ft Nelson)"
  },
  {
    "timezone": "America/Whitehorse",
    "latitude": 60.716667,
    "longitude": -135.05,
    "comments": "MST - Yukon (east)"
  },
  {
    "timezone": "America/Dawson",
    "latitude": 64.716667,
    "longitude": -139.416667,
    "comments": "MST - Yukon (west)"
  },
  {
    "timezone": "America/Vancouver",
    "latitude": 49.266667,
    "longitude": -123.116667,
    "comments": "Pacific - BC (most areas)"
  },
  {
    "timezone": "Indian/Cocos",
    "latitude": -12.166667,
    "longitude": 96.916667
  },
  {
    "timezone": "Europe/Zurich",
    "latitude": 47.383333,
    "longitude": 8.533333,
    "comments": "Swiss time"
  },
  {
    "timezone": "Africa/Abidjan",
    "latitude": 5.316667,
    "longitude": -4.033333
  },
  {
    "timezone": "Pacific/Rarotonga",
    "latitude": -21.233333,
    "longitude": -159.766667
  },
  {
    "timezone": "America/Santiago",
    "latitude": -33.45,
    "longitude": -70.666667,
    "comments": "Chile (most areas)"
  },
  {
    "timezone": "America/Punta_Arenas",
    "latitude": -53.15,
    "longitude": -70.916667,
    "comments": "Region of Magallanes"
  },
  {
    "timezone": "Pacific/Easter",
    "latitude": -27.15,
    "longitude": -109.433333,
    "comments": "Easter Island"
  },
  {
    "timezone": "Asia/Shanghai",
    "latitude": 31.233333,
    "longitude": 121.466667,
    "comments": "Beijing Time"
  },
  {
    "timezone": "Asia/Urumqi",
    "latitude": 43.8,
    "longitude": 87.583333,
    "comments": "Xinjiang Time"
  },
  {
    "timezone": "America/Bogota",
    "latitude": 4.6,
    "longitude": -74.083333
  },
  {
    "timezone": "America/Costa_Rica",
    "latitude": 9.933333,
    "longitude": -84.083333
  },
  {
    "timezone": "America/Havana",
    "latitude": 23.133333,
    "longitude": -82.366667
  },
  {
    "timezone": "Atlantic/Cape_Verde",
    "latitude": 14.916667,
    "longitude": -23.516667
  },
  {
    "timezone": "America/Curacao",
    "latitude": 12.183333,
    "longitude": -69
  },
  {
    "timezone": "Indian/Christmas",
    "latitude": -10.416667,
    "longitude": 105.716667
  },
  {
    "timezone": "Asia/Nicosia",
    "latitude": 35.166667,
    "longitude": 33.366667,
    "comments": "Cyprus (most areas)"
  },
  {
    "timezone": "Asia/Famagusta",
    "latitude": 35.116667,
    "longitude": 33.95,
    "comments": "Northern Cyprus"
  },
  {
    "timezone": "Europe/Prague",
    "latitude": 50.083333,
    "longitude": 14.433333
  },
  {
    "timezone": "Europe/Berlin",
    "latitude": 52.5,
    "longitude": 13.366667,
    "comments": "Germany (most areas)"
  },
  {
    "timezone": "Europe/Copenhagen",
    "latitude": 55.666667,
    "longitude": 12.583333
  },
  {
    "timezone": "America/Santo_Domingo",
    "latitude": 18.466667,
    "longitude": -69.9
  },
  {
    "timezone": "Africa/Algiers",
    "latitude": 36.783333,
    "longitude": 3.05
  },
  {
    "timezone": "America/Guayaquil",
    "latitude": -2.166667,
    "longitude": -79.833333,
    "comments": "Ecuador (mainland)"
  },
  {
    "timezone": "Pacific/Galapagos",
    "latitude": -0.9,
    "longitude": -89.6,
    "comments": "Gal\xE1pagos Islands"
  },
  {
    "timezone": "Europe/Tallinn",
    "latitude": 59.433333,
    "longitude": 24.75
  },
  {
    "timezone": "Africa/Cairo",
    "latitude": 30.05,
    "longitude": 31.25
  },
  {
    "timezone": "Africa/El_Aaiun",
    "latitude": 27.15,
    "longitude": -13.2
  },
  {
    "timezone": "Europe/Madrid",
    "latitude": 40.4,
    "longitude": -3.683333,
    "comments": "Spain (mainland)"
  },
  {
    "timezone": "Africa/Ceuta",
    "latitude": 35.883333,
    "longitude": -5.316667,
    "comments": "Ceuta, Melilla"
  },
  {
    "timezone": "Atlantic/Canary",
    "latitude": 28.1,
    "longitude": -15.4,
    "comments": "Canary Islands"
  },
  {
    "timezone": "Europe/Helsinki",
    "latitude": 60.166667,
    "longitude": 24.95
  },
  {
    "timezone": "Pacific/Fiji",
    "latitude": -18.133333,
    "longitude": 178.416667
  },
  {
    "timezone": "Atlantic/Stanley",
    "latitude": -51.7,
    "longitude": -57.85
  },
  {
    "timezone": "Pacific/Chuuk",
    "latitude": 7.416667,
    "longitude": 151.783333,
    "comments": "Chuuk/Truk, Yap"
  },
  {
    "timezone": "Pacific/Pohnpei",
    "latitude": 6.966667,
    "longitude": 158.216667,
    "comments": "Pohnpei/Ponape"
  },
  {
    "timezone": "Pacific/Kosrae",
    "latitude": 5.316667,
    "longitude": 162.983333,
    "comments": "Kosrae"
  },
  {
    "timezone": "Atlantic/Faroe",
    "latitude": 62.016667,
    "longitude": -6.766667
  },
  {
    "timezone": "Europe/Paris",
    "latitude": 48.866667,
    "longitude": 2.333333
  },
  {
    "timezone": "Europe/London",
    "latitude": 51.508333,
    "longitude": -0.125278
  },
  {
    "timezone": "Asia/Tbilisi",
    "latitude": 41.716667,
    "longitude": 44.783333
  },
  {
    "timezone": "America/Cayenne",
    "latitude": 4.933333,
    "longitude": -52.333333
  },
  {
    "timezone": "Africa/Accra",
    "latitude": 5.55,
    "longitude": -0.216667
  },
  {
    "timezone": "Europe/Gibraltar",
    "latitude": 36.133333,
    "longitude": -5.35
  },
  {
    "timezone": "America/Nuuk",
    "latitude": 64.183333,
    "longitude": -51.733333,
    "comments": "Greenland (most areas)"
  },
  {
    "timezone": "America/Danmarkshavn",
    "latitude": 76.766667,
    "longitude": -18.666667,
    "comments": "National Park (east coast)"
  },
  {
    "timezone": "America/Scoresbysund",
    "latitude": 70.483333,
    "longitude": -21.966667,
    "comments": "Scoresbysund/Ittoqqortoormiit"
  },
  {
    "timezone": "America/Thule",
    "latitude": 76.566667,
    "longitude": -68.783333,
    "comments": "Thule/Pituffik"
  },
  {
    "timezone": "Europe/Athens",
    "latitude": 37.966667,
    "longitude": 23.716667
  },
  {
    "timezone": "Atlantic/South_Georgia",
    "latitude": -54.35,
    "longitude": -36.533333
  },
  {
    "timezone": "America/Guatemala",
    "latitude": 14.633333,
    "longitude": -90.516667
  },
  {
    "timezone": "Pacific/Guam",
    "latitude": 13.466667,
    "longitude": 144.75
  },
  {
    "timezone": "Africa/Bissau",
    "latitude": 11.85,
    "longitude": -15.583333
  },
  {
    "timezone": "America/Guyana",
    "latitude": 6.8,
    "longitude": -58.166667
  },
  {
    "timezone": "Asia/Hong_Kong",
    "latitude": 22.283333,
    "longitude": 114.15
  },
  {
    "timezone": "America/Tegucigalpa",
    "latitude": 14.1,
    "longitude": -87.216667
  },
  {
    "timezone": "America/Port-au-Prince",
    "latitude": 18.533333,
    "longitude": -72.333333
  },
  {
    "timezone": "Europe/Budapest",
    "latitude": 47.5,
    "longitude": 19.083333
  },
  {
    "timezone": "Asia/Jakarta",
    "latitude": -6.166667,
    "longitude": 106.8,
    "comments": "Java, Sumatra"
  },
  {
    "timezone": "Asia/Pontianak",
    "latitude": -0.033333,
    "longitude": 109.333333,
    "comments": "Borneo (west, central)"
  },
  {
    "timezone": "Asia/Makassar",
    "latitude": -5.116667,
    "longitude": 119.4,
    "comments": "Borneo (east, south); Sulawesi/Celebes, Bali, Nusa Tengarra; Timor (west)"
  },
  {
    "timezone": "Asia/Jayapura",
    "latitude": -2.533333,
    "longitude": 140.7,
    "comments": "New Guinea (West Papua / Irian Jaya); Malukus/Moluccas"
  },
  {
    "timezone": "Europe/Dublin",
    "latitude": 53.333333,
    "longitude": -6.25
  },
  {
    "timezone": "Asia/Jerusalem",
    "latitude": 31.780556,
    "longitude": 35.224167
  },
  {
    "timezone": "Asia/Kolkata",
    "latitude": 22.533333,
    "longitude": 88.366667
  },
  {
    "timezone": "Indian/Chagos",
    "latitude": -7.333333,
    "longitude": 72.416667
  },
  {
    "timezone": "Asia/Baghdad",
    "latitude": 33.35,
    "longitude": 44.416667
  },
  {
    "timezone": "Asia/Tehran",
    "latitude": 35.666667,
    "longitude": 51.433333
  },
  {
    "timezone": "Atlantic/Reykjavik",
    "latitude": 64.15,
    "longitude": -21.85
  },
  {
    "timezone": "Europe/Rome",
    "latitude": 41.9,
    "longitude": 12.483333
  },
  {
    "timezone": "America/Jamaica",
    "latitude": 17.968056,
    "longitude": -76.793333
  },
  {
    "timezone": "Asia/Amman",
    "latitude": 31.95,
    "longitude": 35.933333
  },
  {
    "timezone": "Asia/Tokyo",
    "latitude": 35.652778,
    "longitude": 139.744722
  },
  {
    "timezone": "Africa/Nairobi",
    "latitude": -1.283333,
    "longitude": 36.816667
  },
  {
    "timezone": "Asia/Bishkek",
    "latitude": 42.9,
    "longitude": 74.6
  },
  {
    "timezone": "Pacific/Tarawa",
    "latitude": 1.416667,
    "longitude": 173,
    "comments": "Gilbert Islands"
  },
  {
    "timezone": "Pacific/Enderbury",
    "latitude": -3.133333,
    "longitude": -171.083333,
    "comments": "Phoenix Islands"
  },
  {
    "timezone": "Pacific/Kiritimati",
    "latitude": 1.866667,
    "longitude": -157.333333,
    "comments": "Line Islands"
  },
  {
    "timezone": "Asia/Pyongyang",
    "latitude": 39.016667,
    "longitude": 125.75
  },
  {
    "timezone": "Asia/Seoul",
    "latitude": 37.55,
    "longitude": 126.966667
  },
  {
    "timezone": "Asia/Almaty",
    "latitude": 43.25,
    "longitude": 76.95,
    "comments": "Kazakhstan (most areas)"
  },
  {
    "timezone": "Asia/Qyzylorda",
    "latitude": 44.8,
    "longitude": 65.466667,
    "comments": "Qyzylorda/Kyzylorda/Kzyl-Orda"
  },
  {
    "timezone": "Asia/Qostanay",
    "latitude": 53.2,
    "longitude": 63.616667,
    "comments": "Qostanay/Kostanay/Kustanay"
  },
  {
    "timezone": "Asia/Aqtobe",
    "latitude": 50.283333,
    "longitude": 57.166667,
    "comments": "Aqt\xF6be/Aktobe"
  },
  {
    "timezone": "Asia/Aqtau",
    "latitude": 44.516667,
    "longitude": 50.266667,
    "comments": "Mangghysta\u016B/Mankistau"
  },
  {
    "timezone": "Asia/Atyrau",
    "latitude": 47.116667,
    "longitude": 51.933333,
    "comments": "Atyra\u016B/Atirau/Gur'yev"
  },
  {
    "timezone": "Asia/Oral",
    "latitude": 51.216667,
    "longitude": 51.35,
    "comments": "West Kazakhstan"
  },
  {
    "timezone": "Asia/Beirut",
    "latitude": 33.883333,
    "longitude": 35.5
  },
  {
    "timezone": "Asia/Colombo",
    "latitude": 6.933333,
    "longitude": 79.85
  },
  {
    "timezone": "Africa/Monrovia",
    "latitude": 6.3,
    "longitude": -10.783333
  },
  {
    "timezone": "Europe/Vilnius",
    "latitude": 54.683333,
    "longitude": 25.316667
  },
  {
    "timezone": "Europe/Luxembourg",
    "latitude": 49.6,
    "longitude": 6.15
  },
  {
    "timezone": "Europe/Riga",
    "latitude": 56.95,
    "longitude": 24.1
  },
  {
    "timezone": "Africa/Tripoli",
    "latitude": 32.9,
    "longitude": 13.183333
  },
  {
    "timezone": "Africa/Casablanca",
    "latitude": 33.65,
    "longitude": -7.583333
  },
  {
    "timezone": "Europe/Monaco",
    "latitude": 43.7,
    "longitude": 7.383333
  },
  {
    "timezone": "Europe/Chisinau",
    "latitude": 47,
    "longitude": 28.833333
  },
  {
    "timezone": "Pacific/Majuro",
    "latitude": 7.15,
    "longitude": 171.2,
    "comments": "Marshall Islands (most areas)"
  },
  {
    "timezone": "Pacific/Kwajalein",
    "latitude": 9.083333,
    "longitude": 167.333333,
    "comments": "Kwajalein"
  },
  {
    "timezone": "Asia/Yangon",
    "latitude": 16.783333,
    "longitude": 96.166667
  },
  {
    "timezone": "Asia/Ulaanbaatar",
    "latitude": 47.916667,
    "longitude": 106.883333,
    "comments": "Mongolia (most areas)"
  },
  {
    "timezone": "Asia/Hovd",
    "latitude": 48.016667,
    "longitude": 91.65,
    "comments": "Bayan-\xD6lgii, Govi-Altai, Hovd, Uvs, Zavkhan"
  },
  {
    "timezone": "Asia/Choibalsan",
    "latitude": 48.066667,
    "longitude": 114.5,
    "comments": "Dornod, S\xFCkhbaatar"
  },
  {
    "timezone": "Asia/Macau",
    "latitude": 22.197222,
    "longitude": 113.540833
  },
  {
    "timezone": "America/Martinique",
    "latitude": 14.6,
    "longitude": -61.083333
  },
  {
    "timezone": "Europe/Malta",
    "latitude": 35.9,
    "longitude": 14.516667
  },
  {
    "timezone": "Indian/Mauritius",
    "latitude": -20.166667,
    "longitude": 57.5
  },
  {
    "timezone": "Indian/Maldives",
    "latitude": 4.166667,
    "longitude": 73.5
  },
  {
    "timezone": "America/Mexico_City",
    "latitude": 19.4,
    "longitude": -99.15,
    "comments": "Central Time"
  },
  {
    "timezone": "America/Cancun",
    "latitude": 21.083333,
    "longitude": -86.766667,
    "comments": "Eastern Standard Time - Quintana Roo"
  },
  {
    "timezone": "America/Merida",
    "latitude": 20.966667,
    "longitude": -89.616667,
    "comments": "Central Time - Campeche, Yucat\xE1n"
  },
  {
    "timezone": "America/Monterrey",
    "latitude": 25.666667,
    "longitude": -100.316667,
    "comments": "Central Time - Durango; Coahuila, Nuevo Le\xF3n, Tamaulipas (most areas)"
  },
  {
    "timezone": "America/Matamoros",
    "latitude": 25.833333,
    "longitude": -97.5,
    "comments": "Central Time US - Coahuila, Nuevo Le\xF3n, Tamaulipas (US border)"
  },
  {
    "timezone": "America/Mazatlan",
    "latitude": 23.216667,
    "longitude": -106.416667,
    "comments": "Mountain Time - Baja California Sur, Nayarit, Sinaloa"
  },
  {
    "timezone": "America/Chihuahua",
    "latitude": 28.633333,
    "longitude": -106.083333,
    "comments": "Mountain Time - Chihuahua (most areas)"
  },
  {
    "timezone": "America/Ojinaga",
    "latitude": 29.566667,
    "longitude": -104.416667,
    "comments": "Mountain Time US - Chihuahua (US border)"
  },
  {
    "timezone": "America/Hermosillo",
    "latitude": 29.066667,
    "longitude": -110.966667,
    "comments": "Mountain Standard Time - Sonora"
  },
  {
    "timezone": "America/Tijuana",
    "latitude": 32.533333,
    "longitude": -117.016667,
    "comments": "Pacific Time US - Baja California"
  },
  {
    "timezone": "America/Bahia_Banderas",
    "latitude": 20.8,
    "longitude": -105.25,
    "comments": "Central Time - Bah\xEDa de Banderas"
  },
  {
    "timezone": "Asia/Kuala_Lumpur",
    "latitude": 3.166667,
    "longitude": 101.7,
    "comments": "Malaysia (peninsula)"
  },
  {
    "timezone": "Asia/Kuching",
    "latitude": 1.55,
    "longitude": 110.333333,
    "comments": "Sabah, Sarawak"
  },
  {
    "timezone": "Africa/Maputo",
    "latitude": -25.966667,
    "longitude": 32.583333,
    "comments": "Central Africa Time"
  },
  {
    "timezone": "Africa/Windhoek",
    "latitude": -22.566667,
    "longitude": 17.1
  },
  {
    "timezone": "Pacific/Noumea",
    "latitude": -22.266667,
    "longitude": 166.45
  },
  {
    "timezone": "Pacific/Norfolk",
    "latitude": -29.05,
    "longitude": 167.966667
  },
  {
    "timezone": "Africa/Lagos",
    "latitude": 6.45,
    "longitude": 3.4,
    "comments": "West Africa Time"
  },
  {
    "timezone": "America/Managua",
    "latitude": 12.15,
    "longitude": -86.283333
  },
  {
    "timezone": "Europe/Amsterdam",
    "latitude": 52.366667,
    "longitude": 4.9
  },
  {
    "timezone": "Europe/Oslo",
    "latitude": 59.916667,
    "longitude": 10.75
  },
  {
    "timezone": "Asia/Kathmandu",
    "latitude": 27.716667,
    "longitude": 85.316667
  },
  {
    "timezone": "Pacific/Nauru",
    "latitude": -0.516667,
    "longitude": 166.916667
  },
  {
    "timezone": "Pacific/Niue",
    "latitude": -19.016667,
    "longitude": -169.916667
  },
  {
    "timezone": "Pacific/Auckland",
    "latitude": -36.866667,
    "longitude": 174.766667,
    "comments": "New Zealand time"
  },
  {
    "timezone": "Pacific/Chatham",
    "latitude": -43.95,
    "longitude": -176.55,
    "comments": "Chatham Islands"
  },
  {
    "timezone": "America/Panama",
    "latitude": 8.966667,
    "longitude": -79.533333
  },
  {
    "timezone": "America/Lima",
    "latitude": -12.05,
    "longitude": -77.05
  },
  {
    "timezone": "Pacific/Tahiti",
    "latitude": -17.533333,
    "longitude": -149.566667,
    "comments": "Society Islands"
  },
  {
    "timezone": "Pacific/Marquesas",
    "latitude": -9,
    "longitude": -139.5,
    "comments": "Marquesas Islands"
  },
  {
    "timezone": "Pacific/Gambier",
    "latitude": -23.133333,
    "longitude": -134.95,
    "comments": "Gambier Islands"
  },
  {
    "timezone": "Pacific/Port_Moresby",
    "latitude": -9.5,
    "longitude": 147.166667,
    "comments": "Papua New Guinea (most areas)"
  },
  {
    "timezone": "Pacific/Bougainville",
    "latitude": -6.216667,
    "longitude": 155.566667,
    "comments": "Bougainville"
  },
  {
    "timezone": "Asia/Manila",
    "latitude": 14.583333,
    "longitude": 121
  },
  {
    "timezone": "Asia/Karachi",
    "latitude": 24.866667,
    "longitude": 67.05
  },
  {
    "timezone": "Europe/Warsaw",
    "latitude": 52.25,
    "longitude": 21
  },
  {
    "timezone": "America/Miquelon",
    "latitude": 47.05,
    "longitude": -56.333333
  },
  {
    "timezone": "Pacific/Pitcairn",
    "latitude": -25.066667,
    "longitude": -130.083333
  },
  {
    "timezone": "America/Puerto_Rico",
    "latitude": 18.468333,
    "longitude": -66.105833
  },
  {
    "timezone": "Asia/Gaza",
    "latitude": 31.5,
    "longitude": 34.466667,
    "comments": "Gaza Strip"
  },
  {
    "timezone": "Asia/Hebron",
    "latitude": 31.533333,
    "longitude": 35.090556,
    "comments": "West Bank"
  },
  {
    "timezone": "Europe/Lisbon",
    "latitude": 38.716667,
    "longitude": -9.133333,
    "comments": "Portugal (mainland)"
  },
  {
    "timezone": "Atlantic/Madeira",
    "latitude": 32.633333,
    "longitude": -16.9,
    "comments": "Madeira Islands"
  },
  {
    "timezone": "Atlantic/Azores",
    "latitude": 37.733333,
    "longitude": -25.666667,
    "comments": "Azores"
  },
  {
    "timezone": "Pacific/Palau",
    "latitude": 7.333333,
    "longitude": 134.483333
  },
  {
    "timezone": "America/Asuncion",
    "latitude": -25.266667,
    "longitude": -57.666667
  },
  {
    "timezone": "Asia/Qatar",
    "latitude": 25.283333,
    "longitude": 51.533333
  },
  {
    "timezone": "Indian/Reunion",
    "latitude": -20.866667,
    "longitude": 55.466667,
    "comments": "R\xE9union, Crozet, Scattered Islands"
  },
  {
    "timezone": "Europe/Bucharest",
    "latitude": 44.433333,
    "longitude": 26.1
  },
  {
    "timezone": "Europe/Belgrade",
    "latitude": 44.833333,
    "longitude": 20.5
  },
  {
    "timezone": "Europe/Kaliningrad",
    "latitude": 54.716667,
    "longitude": 20.5,
    "comments": "MSK-01 - Kaliningrad"
  },
  {
    "timezone": "Europe/Moscow",
    "latitude": 55.75,
    "longitude": 37.616667,
    "comments": "MSK+00 - Moscow area"
  },
  {
    "timezone": "Europe/Simferopol",
    "latitude": 44.95,
    "longitude": 34.1,
    "comments": "Crimea"
  },
  {
    "timezone": "Europe/Kirov",
    "latitude": 58.6,
    "longitude": 49.65,
    "comments": "MSK+00 - Kirov"
  },
  {
    "timezone": "Europe/Volgograd",
    "latitude": 48.716667,
    "longitude": 44.416667,
    "comments": "MSK+00 - Volgograd"
  },
  {
    "timezone": "Europe/Astrakhan",
    "latitude": 46.35,
    "longitude": 48.05,
    "comments": "MSK+01 - Astrakhan"
  },
  {
    "timezone": "Europe/Saratov",
    "latitude": 51.566667,
    "longitude": 46.033333,
    "comments": "MSK+01 - Saratov"
  },
  {
    "timezone": "Europe/Ulyanovsk",
    "latitude": 54.333333,
    "longitude": 48.4,
    "comments": "MSK+01 - Ulyanovsk"
  },
  {
    "timezone": "Europe/Samara",
    "latitude": 53.2,
    "longitude": 50.15,
    "comments": "MSK+01 - Samara, Udmurtia"
  },
  {
    "timezone": "Asia/Yekaterinburg",
    "latitude": 56.85,
    "longitude": 60.6,
    "comments": "MSK+02 - Urals"
  },
  {
    "timezone": "Asia/Omsk",
    "latitude": 55,
    "longitude": 73.4,
    "comments": "MSK+03 - Omsk"
  },
  {
    "timezone": "Asia/Novosibirsk",
    "latitude": 55.033333,
    "longitude": 82.916667,
    "comments": "MSK+04 - Novosibirsk"
  },
  {
    "timezone": "Asia/Barnaul",
    "latitude": 53.366667,
    "longitude": 83.75,
    "comments": "MSK+04 - Altai"
  },
  {
    "timezone": "Asia/Tomsk",
    "latitude": 56.5,
    "longitude": 84.966667,
    "comments": "MSK+04 - Tomsk"
  },
  {
    "timezone": "Asia/Novokuznetsk",
    "latitude": 53.75,
    "longitude": 87.116667,
    "comments": "MSK+04 - Kemerovo"
  },
  {
    "timezone": "Asia/Krasnoyarsk",
    "latitude": 56.016667,
    "longitude": 92.833333,
    "comments": "MSK+04 - Krasnoyarsk area"
  },
  {
    "timezone": "Asia/Irkutsk",
    "latitude": 52.266667,
    "longitude": 104.333333,
    "comments": "MSK+05 - Irkutsk, Buryatia"
  },
  {
    "timezone": "Asia/Chita",
    "latitude": 52.05,
    "longitude": 113.466667,
    "comments": "MSK+06 - Zabaykalsky"
  },
  {
    "timezone": "Asia/Yakutsk",
    "latitude": 62,
    "longitude": 129.666667,
    "comments": "MSK+06 - Lena River"
  },
  {
    "timezone": "Asia/Khandyga",
    "latitude": 62.650972,
    "longitude": 135.553722,
    "comments": "MSK+06 - Tomponsky, Ust-Maysky"
  },
  {
    "timezone": "Asia/Vladivostok",
    "latitude": 43.166667,
    "longitude": 131.933333,
    "comments": "MSK+07 - Amur River"
  },
  {
    "timezone": "Asia/Ust-Nera",
    "latitude": 64.560833,
    "longitude": 143.226667,
    "comments": "MSK+07 - Oymyakonsky"
  },
  {
    "timezone": "Asia/Magadan",
    "latitude": 59.566667,
    "longitude": 150.8,
    "comments": "MSK+08 - Magadan"
  },
  {
    "timezone": "Asia/Sakhalin",
    "latitude": 46.966667,
    "longitude": 142.7,
    "comments": "MSK+08 - Sakhalin Island"
  },
  {
    "timezone": "Asia/Srednekolymsk",
    "latitude": 67.776944,
    "longitude": 153.56,
    "comments": "MSK+08 - Sakha (E); North Kuril Is"
  },
  {
    "timezone": "Asia/Kamchatka",
    "latitude": 53.016667,
    "longitude": 158.65,
    "comments": "MSK+09 - Kamchatka"
  },
  {
    "timezone": "Asia/Anadyr",
    "latitude": 64.75,
    "longitude": 177.483333,
    "comments": "MSK+09 - Bering Sea"
  },
  {
    "timezone": "Asia/Riyadh",
    "latitude": 24.633333,
    "longitude": 46.716667
  },
  {
    "timezone": "Pacific/Guadalcanal",
    "latitude": -9.533333,
    "longitude": 160.2
  },
  {
    "timezone": "Indian/Mahe",
    "latitude": -4.666667,
    "longitude": 55.483333
  },
  {
    "timezone": "Africa/Khartoum",
    "latitude": 15.6,
    "longitude": 32.533333
  },
  {
    "timezone": "Europe/Stockholm",
    "latitude": 59.333333,
    "longitude": 18.05
  },
  {
    "timezone": "Asia/Singapore",
    "latitude": 1.283333,
    "longitude": 103.85
  },
  {
    "timezone": "America/Paramaribo",
    "latitude": 5.833333,
    "longitude": -55.166667
  },
  {
    "timezone": "Africa/Juba",
    "latitude": 4.85,
    "longitude": 31.616667
  },
  {
    "timezone": "Africa/Sao_Tome",
    "latitude": 0.333333,
    "longitude": 6.733333
  },
  {
    "timezone": "America/El_Salvador",
    "latitude": 13.7,
    "longitude": -89.2
  },
  {
    "timezone": "Asia/Damascus",
    "latitude": 33.5,
    "longitude": 36.3
  },
  {
    "timezone": "America/Grand_Turk",
    "latitude": 21.466667,
    "longitude": -71.133333
  },
  {
    "timezone": "Africa/Ndjamena",
    "latitude": 12.116667,
    "longitude": 15.05
  },
  {
    "timezone": "Indian/Kerguelen",
    "latitude": -49.352778,
    "longitude": 70.2175,
    "comments": "Kerguelen, St Paul Island, Amsterdam Island"
  },
  {
    "timezone": "Asia/Bangkok",
    "latitude": 13.75,
    "longitude": 100.516667,
    "comments": "Indochina (most areas)"
  },
  {
    "timezone": "Asia/Dushanbe",
    "latitude": 38.583333,
    "longitude": 68.8
  },
  {
    "timezone": "Pacific/Fakaofo",
    "latitude": -9.366667,
    "longitude": -171.233333
  },
  {
    "timezone": "Asia/Dili",
    "latitude": -8.55,
    "longitude": 125.583333
  },
  {
    "timezone": "Asia/Ashgabat",
    "latitude": 37.95,
    "longitude": 58.383333
  },
  {
    "timezone": "Africa/Tunis",
    "latitude": 36.8,
    "longitude": 10.183333
  },
  {
    "timezone": "Pacific/Tongatapu",
    "latitude": -21.166667,
    "longitude": -175.166667
  },
  {
    "timezone": "Europe/Istanbul",
    "latitude": 41.016667,
    "longitude": 28.966667
  },
  {
    "timezone": "America/Port_of_Spain",
    "latitude": 10.65,
    "longitude": -61.516667
  },
  {
    "timezone": "Pacific/Funafuti",
    "latitude": -8.516667,
    "longitude": 179.216667
  },
  {
    "timezone": "Asia/Taipei",
    "latitude": 25.05,
    "longitude": 121.5
  },
  {
    "timezone": "Europe/Kiev",
    "latitude": 50.433333,
    "longitude": 30.516667,
    "comments": "Ukraine (most areas)"
  },
  {
    "timezone": "Europe/Uzhgorod",
    "latitude": 48.616667,
    "longitude": 22.3,
    "comments": "Transcarpathia"
  },
  {
    "timezone": "Europe/Zaporozhye",
    "latitude": 47.833333,
    "longitude": 35.166667,
    "comments": "Zaporozhye and east Lugansk"
  },
  {
    "timezone": "Pacific/Wake",
    "latitude": 19.283333,
    "longitude": 166.616667,
    "comments": "Wake Island"
  },
  {
    "timezone": "America/New_York",
    "latitude": 40.714167,
    "longitude": -74.006389,
    "comments": "Eastern (most areas)"
  },
  {
    "timezone": "America/Detroit",
    "latitude": 42.331389,
    "longitude": -83.045833,
    "comments": "Eastern - MI (most areas)"
  },
  {
    "timezone": "America/Kentucky/Louisville",
    "latitude": 38.254167,
    "longitude": -85.761389,
    "comments": "Eastern - KY (Louisville area)"
  },
  {
    "timezone": "America/Kentucky/Monticello",
    "latitude": 36.829722,
    "longitude": -84.849167,
    "comments": "Eastern - KY (Wayne)"
  },
  {
    "timezone": "America/Indiana/Indianapolis",
    "latitude": 39.767778,
    "longitude": -86.1625,
    "comments": "Eastern - IN (most areas)"
  },
  {
    "timezone": "America/Indiana/Vincennes",
    "latitude": 38.676111,
    "longitude": -87.531944,
    "comments": "Eastern - IN (Da, Du, K, Mn)"
  },
  {
    "timezone": "America/Indiana/Winamac",
    "latitude": 41.051389,
    "longitude": -86.608333,
    "comments": "Eastern - IN (Pulaski)"
  },
  {
    "timezone": "America/Indiana/Marengo",
    "latitude": 38.375556,
    "longitude": -86.344722,
    "comments": "Eastern - IN (Crawford)"
  },
  {
    "timezone": "America/Indiana/Petersburg",
    "latitude": 38.491944,
    "longitude": -87.278611,
    "comments": "Eastern - IN (Pike)"
  },
  {
    "timezone": "America/Indiana/Vevay",
    "latitude": 38.747778,
    "longitude": -85.067222,
    "comments": "Eastern - IN (Switzerland)"
  },
  {
    "timezone": "America/Chicago",
    "latitude": 41.85,
    "longitude": -87.65,
    "comments": "Central (most areas)"
  },
  {
    "timezone": "America/Indiana/Tell_City",
    "latitude": 37.953056,
    "longitude": -86.759167,
    "comments": "Central - IN (Perry)"
  },
  {
    "timezone": "America/Indiana/Knox",
    "latitude": 41.295833,
    "longitude": -86.625,
    "comments": "Central - IN (Starke)"
  },
  {
    "timezone": "America/Menominee",
    "latitude": 45.108056,
    "longitude": -87.615833,
    "comments": "Central - MI (Wisconsin border)"
  },
  {
    "timezone": "America/North_Dakota/Center",
    "latitude": 47.116389,
    "longitude": -101.299167,
    "comments": "Central - ND (Oliver)"
  },
  {
    "timezone": "America/North_Dakota/New_Salem",
    "latitude": 46.845,
    "longitude": -101.410833,
    "comments": "Central - ND (Morton rural)"
  },
  {
    "timezone": "America/North_Dakota/Beulah",
    "latitude": 47.264167,
    "longitude": -101.777778,
    "comments": "Central - ND (Mercer)"
  },
  {
    "timezone": "America/Denver",
    "latitude": 39.739167,
    "longitude": -104.984167,
    "comments": "Mountain (most areas)"
  },
  {
    "timezone": "America/Boise",
    "latitude": 43.616667,
    "longitude": -116.2,
    "comments": "Mountain - ID (south); OR (east)"
  },
  {
    "timezone": "America/Phoenix",
    "latitude": 33.448889,
    "longitude": -112.073333,
    "comments": "MST - Arizona (except Navajo)"
  },
  {
    "timezone": "America/Los_Angeles",
    "latitude": 34.052222,
    "longitude": -118.242778,
    "comments": "Pacific"
  },
  {
    "timezone": "America/Anchorage",
    "latitude": 61.218056,
    "longitude": -149.900278,
    "comments": "Alaska (most areas)"
  },
  {
    "timezone": "America/Juneau",
    "latitude": 58.301944,
    "longitude": -134.419722,
    "comments": "Alaska - Juneau area"
  },
  {
    "timezone": "America/Sitka",
    "latitude": 57.176389,
    "longitude": -135.301944,
    "comments": "Alaska - Sitka area"
  },
  {
    "timezone": "America/Metlakatla",
    "latitude": 55.126944,
    "longitude": -131.576389,
    "comments": "Alaska - Annette Island"
  },
  {
    "timezone": "America/Yakutat",
    "latitude": 59.546944,
    "longitude": -139.727222,
    "comments": "Alaska - Yakutat"
  },
  {
    "timezone": "America/Nome",
    "latitude": 64.501111,
    "longitude": -165.406389,
    "comments": "Alaska (west)"
  },
  {
    "timezone": "America/Adak",
    "latitude": 51.88,
    "longitude": -176.658056,
    "comments": "Aleutian Islands"
  },
  {
    "timezone": "Pacific/Honolulu",
    "latitude": 21.306389,
    "longitude": -157.858333,
    "comments": "Hawaii"
  },
  {
    "timezone": "America/Montevideo",
    "latitude": -34.901111,
    "longitude": -56.164444
  },
  {
    "timezone": "Asia/Samarkand",
    "latitude": 39.666667,
    "longitude": 66.8,
    "comments": "Uzbekistan (west)"
  },
  {
    "timezone": "Asia/Tashkent",
    "latitude": 41.333333,
    "longitude": 69.3,
    "comments": "Uzbekistan (east)"
  },
  {
    "timezone": "America/Caracas",
    "latitude": 10.5,
    "longitude": -66.933333
  },
  {
    "timezone": "Asia/Ho_Chi_Minh",
    "latitude": 10.75,
    "longitude": 106.666667,
    "comments": "Vietnam (south)"
  },
  {
    "timezone": "Pacific/Efate",
    "latitude": -17.666667,
    "longitude": 168.416667
  },
  {
    "timezone": "Pacific/Wallis",
    "latitude": -13.3,
    "longitude": -176.166667
  },
  {
    "timezone": "Pacific/Apia",
    "latitude": -13.833333,
    "longitude": -171.733333
  },
  {
    "timezone": "Africa/Johannesburg",
    "latitude": -26.25,
    "longitude": 28
  }
];

// src/lib/timezone.ts
var timezoneAbbreviations = {
  "gmt": "Etc/GMT",
  "utc": "Etc/UTC",
  "est": "America/New_York",
  "edt": "America/New_York",
  "cst": "America/Chicago",
  "cdt": "America/Chicago",
  "mst": "America/Denver",
  "mdt": "America/Denver",
  "pst": "America/Los_Angeles",
  "pdt": "America/Los_Angeles",
  "akst": "America/Anchorage",
  "akdt": "America/Anchorage",
  "hst": "Pacific/Honolulu",
  "aest": "Australia/Sydney",
  "aedt": "Australia/Sydney",
  "acst": "Australia/Darwin",
  "acdt": "Australia/Adelaide",
  "awst": "Australia/Perth",
  "bst": "Europe/London",
  "cet": "Europe/Paris",
  "cest": "Europe/Paris",
  "wet": "Europe/Lisbon",
  "wemt": "Europe/Lisbon",
  "eet": "Europe/Athens",
  "eest": "Europe/Athens",
  "ist": "Asia/Kolkata",
  "pkst": "Asia/Karachi",
  "pst_ph": "Asia/Manila",
  "pht": "Asia/Manila",
  "kst": "Asia/Seoul",
  "jst": "Asia/Tokyo",
  "sast": "Africa/Johannesburg",
  "cat": "Africa/Maputo",
  "eat": "Africa/Nairobi",
  "wast": "Africa/Lagos",
  "wat": "Africa/Lagos",
  "wib": "Asia/Jakarta",
  "wita": "Asia/Makassar",
  "wit": "Asia/Jayapura",
  "gulf": "Asia/Dubai",
  "ast": "America/Puerto_Rico",
  "adt": "America/Halifax",
  "nst": "America/St_Johns",
  "ndt": "America/St_Johns",
  "brt": "America/Sao_Paulo",
  "art": "America/Argentina/Buenos_Aires",
  "pyt": "America/Asuncion",
  "uyt": "America/Montevideo",
  "clt": "America/Santiago",
  "vlst": "America/Caracas",
  "msk": "Europe/Moscow",
  "omsk": "Asia/Omsk",
  "yeet": "Asia/Yekaterinburg",
  "krast": "Asia/Krasnoyarsk",
  "irkt": "Asia/Irkutsk",
  "yakt": "Asia/Yakutsk",
  "vlat": "Asia/Vladivostok",
  "chot": "Asia/Choibalsan"
};
var allTimezones = [
  ...Intl.supportedValuesOf("timeZone").map((tz) => ({ name: tz, canonical: tz }))
];
function findClosestTimezone(query) {
  const normalizedQuery = query.toLowerCase();
  const exactMatch = allTimezones.find((tz) => {
    const [, city] = tz.name.toLowerCase().split("/");
    return tz.name.toLowerCase() === normalizedQuery || city === normalizedQuery;
  });
  if (exactMatch) {
    return exactMatch.canonical;
  }
  let closestMatch;
  let smallestDistance = Infinity;
  allTimezones.forEach((tz) => {
    const [, city] = tz.name.toLowerCase().split("/");
    const distance = import_fast_levenshtein.default.get(city, normalizedQuery);
    if (distance < smallestDistance) {
      smallestDistance = distance;
      closestMatch = tz;
    }
  });
  return timezoneAbbreviations[normalizedQuery] ?? (closestMatch == null ? void 0 : closestMatch.canonical);
}
function convertTime(fromZoneRaw, toZonesRaw, fromTimeRaw, fromDateRaw) {
  const fromZone = findClosestTimezone(fromZoneRaw);
  const fromTime = decodeURIComponent(fromTimeRaw || import_luxon.DateTime.now().setZone(fromZone).toFormat("HH:mm"));
  const fromDate = decodeURIComponent(fromDateRaw || import_luxon.DateTime.now().setZone(fromZone).toFormat("yyyy-MM-dd"));
  const fromDateTime = import_luxon.DateTime.fromFormat(`${fromDate} ${fromTime}`, "yyyy-MM-dd HH:mm", {
    zone: fromZone
  });
  if (!fromDateTime.isValid) {
    throw new Error("Invalid time format");
  }
  const { latitude, longitude } = timezoneLocations.find((location) => location.timezone === fromZone) || {
    latitude: 0,
    longitude: 0
  };
  const fromPin = {
    name: fromZone || "",
    latitude,
    longitude,
    time: fromDateTime.toFormat("yyyy-MM-dd HH:mm z"),
    date: fromDateTime.toISO(),
    isFrom: true
  };
  const toZones = toZonesRaw.map(findClosestTimezone);
  const toPins = toZones.map((toZone) => {
    const {
      latitude: latitude2,
      longitude: longitude2
    } = timezoneLocations.find((location) => location.timezone === toZone) || { latitude: 0, longitude: 0 };
    const date = fromDateTime.setZone(toZone);
    const toDateTimeString = date.toFormat("yyyy-MM-dd HH:mm z");
    return {
      name: toZone,
      latitude: latitude2,
      longitude: longitude2,
      time: toDateTimeString,
      date: date.toISO(),
      isFrom: false
    };
  });
  toPins.push(fromPin);
  return toPins;
}

// src/components/TimelineMap.tsx
function MapFitter({ pins }) {
  const map = (0, import_react_leaflet.useMap)();
  import_react.default.useEffect(() => {
    if (!map || pins.length === 0) return;
    const validPins = pins.filter(
      (p) => typeof p.latitude === "number" && typeof p.longitude === "number"
    );
    if (validPins.length === 0) return;
    const bounds = validPins.map((p) => [p.latitude, p.longitude]);
    if (bounds.length === 1) {
      map.setView(bounds[0], 5);
    } else {
      map.fitBounds(bounds, { padding: [40, 40] });
    }
  }, [map, pins]);
  return null;
}
function TimelineMap({
  fromZone,
  toZones,
  time,
  date,
  theme = "dark",
  onPinClick
}) {
  const [pins, setPins] = import_react.default.useState(
    () => convertTime(fromZone, toZones, time, date)
  );
  import_react.default.useEffect(() => {
    setPins(convertTime(fromZone, toZones, time, date));
  }, [fromZone, toZones, time, date]);
  const divIcon = (0, import_react.useMemo)(
    () => import_leaflet.default.divIcon({
      className: "custom-icon",
      html: `<div class="circle"></div>`
    }),
    []
  );
  const handlePinClick = (pin) => {
    setPins((prev) => {
      const updated = prev.map((p) => ({ ...p, isFrom: false }));
      const idx = updated.findIndex(
        (p) => p.latitude === pin.latitude && p.longitude === pin.longitude
      );
      if (idx !== -1) updated[idx] = { ...updated[idx], isFrom: true };
      return updated;
    });
    onPinClick == null ? void 0 : onPinClick(pin);
  };
  return (
    // Fix 2: Wrapper div with .timeline-map class to scope CSS
    /* @__PURE__ */ import_react.default.createElement("div", { className: "timeline-map" }, /* @__PURE__ */ import_react.default.createElement(
      import_react_leaflet.MapContainer,
      {
        zoom: 3,
        center: [0, 0],
        className: theme === "dark" ? "timeline-map-dark" : "",
        style: { height: "100vh", width: "100%" }
      },
      /* @__PURE__ */ import_react.default.createElement(
        import_react_leaflet.TileLayer,
        {
          url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
          attribution: '\xA9 <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'
        }
      ),
      /* @__PURE__ */ import_react.default.createElement(MapFitter, { pins }),
      /* @__PURE__ */ import_react.default.createElement(import_react_leaflet.FeatureGroup, null, pins.map((pin, index) => {
        const dt = import_luxon2.DateTime.fromISO(pin.date, { setZone: true });
        return /* @__PURE__ */ import_react.default.createElement(
          import_react_leaflet.Marker,
          {
            key: index,
            position: [pin.latitude, pin.longitude],
            icon: divIcon
          },
          /* @__PURE__ */ import_react.default.createElement(import_react_leaflet.Popup, { autoClose: false, closeButton: false, closeOnClick: false }, pin.isFrom ? /* @__PURE__ */ import_react.default.createElement("h1", null, pin.time.split(" ").pop(), /* @__PURE__ */ import_react.default.createElement("br", null), dt.toFormat("yyyy-MM-dd HH:mm")) : /* @__PURE__ */ import_react.default.createElement("h3", { onClick: () => handlePinClick(pin) }, pin.time.split(" ").pop(), /* @__PURE__ */ import_react.default.createElement("br", null), dt.toFormat("yyyy-MM-dd HH:mm")))
        );
      }))
    ))
  );
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  TimelineMap,
  convertTime,
  findClosestTimezone
});
//# sourceMappingURL=index.js.map